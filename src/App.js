...
function App() {
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(moveSnake, 20000);
  ...
  return (
    <div className="text-center">
      ...
      {gameOver ? (
        ...
        <div className="inline-block m-5 border-2 border-solid border-black">
            ...
            <div key={rowIndex} className="flex">
              {Array.from({ length: gridSize }, (_, colIndex) => {
                ...
                return (
                  <div
                    key={colIndex}
                    className={`w-5 h-5 border-1 border-solid border-white ${
                      isSnake ? "snake bg-black" : ""
                    } ${isFood ? "food bg-red-600" : ""}`}></div>
                );
              })}
            </div>
          ))}
